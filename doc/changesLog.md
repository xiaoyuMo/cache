...
* ####4.13 修改说明
    ...
    * com.xiaoyu.cache.clone.ICloner中的Object deepClone(Object obj) 改为 Object deepClone(Object obj, final Type type)，在能获得Type的情况下使用，能更好保证数据准备和效率
...
* ####4.7 修改说明：
    ...
    * 实现表达式引擎的可扩展性，默认还是使用SpringEL表达式引擎（com.xiaoyu.cache.script.SpringELParser），但可以自己实际情况进行扩展。扩展方法：实现com.xiaoyu.cache.script.IScriptParser,然后通过配置把实现的实例注入到AbstractCacheManager中的scriptParser属性即可。使用不同表达式引擎时，注意表达式的语言的区别。
    * 增加了JavaScript 表达式引擎实现：com.xiaoyu.cache.script.JavaScriptParser。如果项目使用JDK1.8的，想用JavaScript的话，建议使用Nashorn 来实现，性能会比较好。
...
* ####4.4 修改说明：
    ...
    com.xiaoyu.cache.map.CachePointCut 做如下修改
...
* ####4.0 实现AOP的可扩展
    ...
    * 实现了使用Aspectj进行AOP拦截：com.xiaoyu.cache.aop.aspectj.AspectjAopInterceptor
...
* ####3.7 细节优化：
    ...
    * 将com.xiaoyu.cache.map.CachePointCut.shutDown() 改成  destroy()
...
* ####3.0 此版本做了大调整，有些功能已经不兼容老版本 
    ...
    如果在开发阶段不想使用Redis来缓存数据，可以使用com.xiaoyu.cache.map.CachePointCut，把数据缓存到本地内存中，虽然它不支持使用通配符进行批量删除缓存，但同样支持使用hash表进行批量删除缓存。所以转用Redis缓存数据是没有任务问题的。

    如果需要在MyBatis Mapper中使用@Cache和@CacheDelete，则需要使用com.xiaoyu.cache.mybatis.CachePointCutProxy 来处理。
...
* ####2.8 com.xiaoyu.lib.util.BeanUtil.toString()方法中增加反射缓存，提升反射效率
...
* ####2.0 增加了Hessian 和 Kryo 序列化支持，还是使用JDK自带的处理方法。修改方法如下：
        ...
        <bean id="cachePointCut" class="com.xiaoyu.cache.redis.ShardedCachePointCut" destroy-method="destroy">
