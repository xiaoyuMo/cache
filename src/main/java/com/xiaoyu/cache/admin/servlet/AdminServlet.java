...
import com.xiaoyu.lib.util.BeanUtil;
...
public class AdminServlet extends HttpServlet {
	...
	public void init(ServletConfig servletConfig) throws ServletException {
		...
		if (null != _cacheManagerConfig && _cacheManagerConfig.length() > 0) {
			...
			throw new ServletException("请设置com.xiaoyu.cache.admin.servlet.AdminServlet 中的 cacheManagerConfig 参数！");
		}
	}
	...
	private void printHtmlHead(HttpServletResponse resp, String cacheManagerName) throws IOException {
		...
		html.append(
				"td { border-right: 1px solid #555555; border-bottom: 1px solid #555555; text-align: center; line-height: 24px; word-wrap: break-word; }");
		html.append("</style>");
	...
	private void printForm(HttpServletResponse resp, String cacheManagerName, String cacheManagerNames[])
			throws IOException {
		StringBuilder html = new StringBuilder();
	...
	private void printList(HttpServletRequest req, HttpServletResponse resp, AbstractCacheManager cacheManager,
		...
		for (AutoLoadTO tmpTO : queue) {
			...
			html.append(
					"    <td>" + getDateFormat(tmpTO.getLastRequestTime() + tmpTO.getCache().requestTimeout() * 1000)
			...
			if (null != tmpTO.getArgs() && tmpTO.getArgs().length > 0) {
				html.append("<a href=\"" + req.getContextPath() + req.getServletPath()
						+ "?act=showArgs&cacheManagerName=" + cacheManagerName + "&cacheKey=" + _key + "&hfield="
						+ _hfield + "\" target=\"_blank\">show args values</a>");
			}
			html.append("</td>");
			html.append("  </tr>");
		}
